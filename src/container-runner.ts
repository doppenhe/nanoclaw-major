...
async function finish(sessionId: string, runtime: ActiveSessionRuntime, failure?: SessionFailure): Promise<void> {
  ...
  // FORK-LOCAL — drop the session's GitHub token file. The next spawn rewrites
  // it from `gh auth token`, so nothing depends on it surviving, and leaving it
  // on disk would keep a live credential around for every session that ever ran.
  cleanupGithubTokenFile(sessionId);
...
export async function buildMounts(
  ...
  // LOCAL FORK — global memory directory, shared by every agent group. Read-only
  // by default, read-write when it is a git checkout (the doppenhe/major_wiki
  // workflow carried over from v1). Classed 'allowlisted-extra': it lives under
  // GROUPS_DIR but outside any single `groups/<folder>` subtree, so 'group-state'
  // would be denied by the folder-label scope pin, and it is not a release
  // surface. `groupScope` is still stamped with the session's group for lineage.
  ...
  // FORK-LOCAL — GitHub push auth, composed last so it sees EVERY writable
  // mount above, the operator's additional mounts included: those are how a
  // doppenhe/* checkout usually reaches a container, and deciding before them
  // would miss it. See src/fork-github-auth.ts.
  const githubTokenMount = composeGithubTokenMount(mounts, agentGroup.id, session.id);
  if (githubTokenMount) mounts.push(githubTokenMount);
